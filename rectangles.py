...
for x in range(0,int(screen_x/2),step):
    for y in range(0,int(screen_y/2),step):
        if x == y:
            print(f"{x},{y}", end=' ')
            color[color_select]['canvas'].rectangle([(x,y),(screen_x-x,screen_y-y)], fill=color[color_select]['fill'])
            if color_select == 1:
                color[3]['canvas'].rectangle([(x+step,y+step),(screen_x-(x+step),screen_y-(y+step))], fill=color[3]['fill'])
